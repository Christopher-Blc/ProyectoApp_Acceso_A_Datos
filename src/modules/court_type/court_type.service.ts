...
export class CourtTypeService {
  constructor(
    @InjectRepository(TipoCourt)
    private readonly tipoPistaRepository: Repository<TipoCourt>,
  ) {}
  ...
  // Obtener uno por ID
  async findOne(id: number): Promise<TipoCourt> {
  ...
  // Crear un nuevo tipo
  async create(dto: TipoCourtDto, imagenFilename?: string): Promise<TipoCourt> {
  ...
  async update(id: number, dto: UpdateTipoCourtDto, imagenFilename?: string): Promise<TipoCourt> {
