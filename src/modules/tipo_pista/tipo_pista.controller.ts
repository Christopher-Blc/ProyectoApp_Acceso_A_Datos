...
export class TipoPistaController {
    ...
    constructor(private readonly tipoPistaService: TipoPistaService) {}
          ...
          @ApiResponse({ status: 200, description: 'Court type retrieved successfully.' })
          ...
          @ApiResponse({ status: 404, description: 'Court type not found.' })
          @ApiParam({ name: 'id', example: 1 })
          async update(@Param('id') id: number, @Body() tipoPistaDto: TipoPistaDto): Promise<TipoPista | null> {
            try {
